...
public class Day03_5 { // cs
	...
	public static void main(String[] args) { // ms
		...
		// p.121 예제1 : 1부터 10까지 출력
		
		for(int i = 1 ; i <= 10; i++) {
			// int i = 1 -> 초기값 선언[i는 반복변수]
			// 	 i <= 10 -> 조건식 : i가 10 이하이면 실행, 아니면 실행 안함
		...
		int sum = 0;										// 누적합 출력할 변수
		for(int i = 1; i <= 100; i++) {						// i는 1부터 100까지 1씩 증가하면서 실행문 반복처리
			sum += i;										// sum = sum + i
		}													// 반복문 종료
		System.out.println("1~100까지 누적합 결과 : " + sum);
		
		// 예제3 : 1부터 100까지 7배수의 누적합계 = i는 0부터 100까지 7씩 증가하면서 실행문 반복 처리
		int sum2 = 0;
		for (int i = 0; i <= 100; i += 7) {
			sum2 += i;
		}
		System.out.println("1~100까지 7의배수 누적합 결과 : " + sum2);
		
		// 예제3 : 1부터 100까지 7배수의 누적합계 
		int sum3 = 0;
		for( int i = 1 ; i<=100 ; i++ ) { 					// 배수찾기 : 값 % 수 == 0 이면 값 그 수의 배수 			
			if( i % 7 == 0 ) sum3 += i; 					// 만약에 i가 7의배수이면 누적합 아니면 x	
		}
