...
public class Day19_3 {
	...
	public static void main(String[] args) {
		...
		map.put("홍길동", 95);			System.out.println("확인 : " + map);		// key는 중복 불가능, 기존 key는 사라직고 새로운 key로 대체
		map.put("유재석", 95);			System.out.println("확인 : " + map);
		
		// 3. map객체명.size() : map객체 내 엔트리 (키=값)의 개수
		System.out.println("map 객체 내 엔트리 수 : " + map.size());
		
		// 4. map객체명.get(key) : map객체내 키 -> 값 호출
		System.out.println("map 객체 내 값 호출 (키 호출시 값 호출) : " + map.get("유재석"));
		
		// 5. 모든 key 호출
		System.out.println("map 객체 내 모든 키 호출 : " + map.keySet());
		
		// 6. 모든 value 호출
		System.out.println("map 객체 내 모든 값 호출 : " + map.values());
		
		// 7. key 반복문 이용해서 모든 value 호출 가능
